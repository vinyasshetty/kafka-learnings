...
* **log.retention.bytes** =&gt; Now this determines how big each partition can get before deleting the partition.Say this is set to 2000000000 \(2 GB\) ,and that topic has 10 partitions,then that topic can have at the max 20GB of data.But the tracking and deleting is done in terms of partition ie if a partition size exceeds greater then 2gb then its cleaned up.**Does this NOT wait until file handle is closed,because if its waiting then how can it delete because size will never grow after file handle closing???\[Answer : It waits for file handle and for this scenario it neever cleans up the data  until it satisifies log.retention.ms if given\]**
